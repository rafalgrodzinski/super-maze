...
static float METLToRadians(const float& degrees)
{
...
simd::float4x4 METL::scale(const float& x,
                           const float& y,
                           const float& z)
{
...
simd::float4x4 METL::scale(const simd::float3& s)
{
...
simd::float4x4 METL::translate(const float& x,
                               const float& y,
                               const float& z)
{
...
simd::float4x4 METL::translate(const simd::float3& t)
{
    return METL::translate(t.x, t.y, t.z);
} // translate

#pragma mark -
#pragma mark Public - Transformations - Rotate

simd::float4x4 METL::rotate(const float& angle,
                            const simd::float3& r)
{
    float a = METLToRadians(angle);
    float c = std::cos(a);
    float s = std::sin(a);
    float k = 1.0f - c;
                            ...
                            const float& z)
{
    ...
    return METL::rotate(angle, r);
} // rotate

#pragma mark -
#pragma mark Public - Transformations - Perspective

simd::float4x4 METL::perspective(const float& width,
                                 const float& height,
                                 const float& near,
                                 const float& far)
{
...
simd::float4x4 METL::perspective_fov(const float& fovy,
                                     const float& aspect,
                                     const float& near,
                                     const float& far)
{
    float angle  = METLToRadians(0.5f * fovy);
                                     ...
                                     const float& far)
{
    ...
    return METL::perspective_fov(fovy, aspect, near, far);
} // perspective_fov

#pragma mark -
#pragma mark Public - Transformations - LookAt

simd::float4x4 METL::lookAt(const simd::float3& eye,
...
simd::float4x4 METL::lookAt(const float * const pEye,
                            ...
                            const float * const pUp)
{
    ...
    return METL::lookAt(eye, center, up);
} // lookAt

#pragma mark -
#pragma mark Public - Transformations - Orthographic

simd::float4x4 METL::ortho2d(const float& left,
                             const float& right,
                             const float& bottom,
                             const float& top,
                             const float& near,
...
simd::float4x4 METL::ortho2d(const simd::float3& origin,
                             const simd::float3& size)
{
    return METL::ortho2d(origin.x, origin.y, origin.z, size.x, size.y, size.z);
} // ortho2d

#pragma mark -
#pragma mark Public - Transformations - Off-Center Orthographic

simd::float4x4 METL::ortho2d_oc(const float& left,
                                const float& right,
                                const float& bottom,
                                const float& top,
                                const float& near,
...
simd::float4x4 METL::ortho2d_oc(const simd::float3& origin,
                                const simd::float3& size)
{
    return METL::ortho2d_oc(origin.x, origin.y, origin.z, size.x, size.y, size.z);
} // ortho2d_oc

#pragma mark -
#pragma mark Public - Transformations - frustum

simd::float4x4 METL::frustum(const float& fovH,
                             const float& fovV,
                             const float& near,
                             const float& far)
{
    float width  = 1.0f / std::tan(METLToRadians(0.5f * fovH));
    float height = 1.0f / std::tan(METLToRadians(0.5f * fovV));
...
simd::float4x4 METL::frustum(const float& left,
                             const float& right,
                             const float& bottom,
                             const float& top,
                             const float& near,
...
simd::float4x4 METL::frustum_oc(const float& left,
                                const float& right,
                                const float& bottom,
                                const float& top,
                                const float& near,
